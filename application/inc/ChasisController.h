...
#define GIMBAL_FOLLOW_ZERO 7362 // 底盘跟随机械零点
...
typedef enum
{
  ...
  Preset_1,     // 预设模式1
  Preset_2,     // 预设模式2
} Sentry_Preset_Mode_e;

  // 在ChasisController.h或相关头文件中添加平滑参数
typedef struct {
    float smooth_x_v;      // 平滑后的X速度
    float smooth_y_v;      // 平滑后的Y速度  
    float smooth_yaw_v;    // 平滑后的YAW速度
    float smooth_factor;   // 平滑系数 (0-1)
} velocity_smoother_t;

typedef struct Chassis_Control_t
{
  /*  传感器信息 */
...
//  PowerLimitMethod power_limit_method;
  ...
  float ActionTask_Time;
  float Preset_Time_Point; // 预设模式时间点
  float Preset_Time; // 预设模式时间
  int cycle_position; // 周期位置

  uint8_t preset_mode;
  uint8_t last_preset_mode; // 上一次预设模式

  velocity_smoother_t vel_smoother;
} Chassis_Control_t;

extern Chassis_Control_t chassis_controller;

void InfantryInit(Chassis_Control_t *chassis_controller);
void main_control(Chassis_Control_t *chassis_controller);
void wheels_power_limit(Chassis_Control_t *chassis_controller);
void execute_control(ExcuteTorque *torque);
void get_sensors_info(Sensors *sensors_info);

// 设置机器人功率以及控制其速度
void set_robot_speed(Chassis_Control_t *chassis_controller);
