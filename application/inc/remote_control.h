...
enum CONTROL_MODE_ACTION
{
    ...
    ROTATE_WITH_WORLD_COORDINATE, // 底盘自转同时沿世界坐标系平移
};
...
typedef struct
{
    ...
    unsigned short Previous_rc_Left_SW; // 前一刻的左边拨杆
} Remote;
...
typedef struct RemoteController
{
    ...
    enum FlyControlState fly_state;
    enum CHASSIS_FORMAT chassis_format; // 底盘形状
